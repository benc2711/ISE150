...
#I know how to code which is why I used functions in my code. It makes it much cleaner for me to code and read.
def encrypt(fmessage):
    encrypted = ""
    for i in range(0,len(fmessage),1):
        newAlphabet = "EHTGDBWIUQRXLMVFSJCPYZKAON"
        if ord(fmessage[i]) > 96 and ord(fmessage[i])<122:
            inAlphabet = ord(fmessage[i])-97
            encrypted += newAlphabet[inAlphabet].lower()
        elif ord(fmessage[i]) > 64 and ord(fmessage[i])<91:
            inAlphabet = ord(fmessage[i])-65
            encrypted += newAlphabet[inAlphabet]
        else:
            encrypted += fmessage[i]
